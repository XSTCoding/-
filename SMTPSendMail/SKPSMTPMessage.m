...
- (BOOL)send
{
    ...
    NSLog(@"C  1: Attempting to connect to server at: %@:%d", relayHost, relayPort);
    [delegate messageSendInfo:self sendInfo:[NSString stringWithFormat:@"C  1: Attempting to connect to server at: %@:%d", relayHost, relayPort]];
...
- (void)parseBuffer
{
    ...
    while (![scanner isAtEnd])
    {
        ...
        if (foundLine)
        {
            ...
            [delegate messageState:self sendState:sendState];
            switch (sendState)
            {
...
- (BOOL)sendParts
{
    ...
    NSLog(@"C  14: %s  , lenght:%d", [messageData bytes], [messageData length]);
